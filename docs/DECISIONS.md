...
| **D26** | **Frontier Elo v2 — category-attenuated battle weights (2026-07-24).** Revises D21's battle construction only; outcomes stay binary and the MM fit, λ=1 anchor, scale and convergence contract are unchanged. Problem: battles counted every shared benchmark equally, so a pair sharing many benchmarks in ONE domain let that domain decide the pairing linearly (e.g. a coding-heavy pair was effectively a coding rating), and leave-one-CATEGORY-out influence on the top-50 spanned 0.36–3.74 mean \|Δrank\| across domains — the rating under-represented generalization. Fix: weight each battle `1/√n_c`, where `n_c` = how many benchmarks of that battle's category the PAIR shares — a domain with n shared benchmarks contributes √n total match weight (more evidence still counts for more, but sub-linearly). Pair-local, bounds-free, unit-agnostic, deterministic. **Measured on the live corpus (543 models):** meaningful category-influence spread halves (0.64–3.46 vs 0.36–3.74 mean \|Δrank\| leave-one-category-out, excluding the near-empty human-preference bucket); single-benchmark leave-one-out influence: mean-shift improves (avg top-50 0.393 vs 0.403) while max-shift ticks up slightly (3.12 vs 2.88 — far below linear equalization's 3.51); era coherence 13/13 pinned orderings; specialist guard holds (DeepSeek-Prover-V2 stays ~rank 52, not inflated); top-5 membership invariant across λ ∈ {0.5, 1, 2, 4} (order stable for λ ≥ 1; at λ = 0.5 the ranks-4/5 near-tie — a 0.2-Elo gap — swaps). **Rejected with evidence:** *linear* per-category equalization `1/n_c` (a lone benchmark in a sparse category inherits a full category vote — avg max leave-one-benchmark-out shift worsens 2.88→3.51; same failure D21 documented for per-benchmark equalization); *margin-aware soft outcomes* (logistic on robust-MAD-scaled gaps) and *noise tie-bands* (\|Δ\| ≤ 0.15–0.3·MAD = draw) — both systematically inflate blowout specialists because the frontier's genuine small-margin edges dilute while a specialist crushing a weak field on a few niche benchmarks keeps a near-perfect record (DeepSeek-Prover-V2-671B rockets to #1 under soft margins, rank 43/27 under tie bands vs 52 baseline); margin machinery measures dominance, not generalization, confirming D21's original deferral for the deeper reason. Engine: `buildBattles` now takes `{slug, category}` refs and runs two deterministic passes (per-pair category counts, then attenuated accumulation); the independent scratch tally in `derive.test.ts` mirrors the weighting; `bradley-terry.test.ts` pins the 1/√n_c attenuation analytically. The methodology page documents the weighting publicly. |
| **D27** | **Pareto frontier on the quality-vs-price scatter (2026-07-24).** The dashboard "Quality vs. price" scatter (Frontier Elo vs. output $/Mtok, log-x) gains an **efficient-frontier guide line**: the subset of plotted models that no other model *dominates*, where a model dominates another when it is BOTH no more expensive AND no lower Elo. Because cheaper and higher-Elo are both "better", the efficient set is the plot's **upper-left envelope** — swept left→right in ascending price it is exactly the running maximum of quality, so `paretoFrontier` (pure, in `charts/scales.ts`, unit-tested) resolves it in a single pass over the price-sorted points and returns the vertices in ascending-price order, ready to string into a monotone `<polyline>`. Rendering (`charts/scatter.tsx`): the line is drawn in `--acc` **beneath the dots** (a frontier model still reads as its own dot with the line passing through it), `pointerEvents:none` so it never intercepts the nearest-point hover, and a static (non-filter) `Pareto frontier` key sits in the legend alongside the Open/Closed toggles. **Scoped to the visible set:** the frontier is derived from the points actually plotted, so toggling a weight camp re-fits it exactly as it re-fits the `fitYWindow` y-axis — hide "Closed" and the line snaps to the open-weights frontier. Tie handling is exact: models sharing a price collapse to the single highest-Elo one, and a costlier model that only *equals* an earlier model's Elo is dominated and dropped (strictly-greater test), so x strictly increases and Elo strictly increases along the rendered line. Pure geometry + display only — no data, scoring, or snapshot change (extends the C6 chart-math contract). |
| **D28** | **Compare v2 — unified spec sheet, 4 slots, verdict + field-context rail (2026-09-25).** Supersedes D5's "3 slots (A/B/C, fixed colors)". Compare is now a single dense spec sheet (`compare-table.tsx`) — sticky model header (name·tag·Elo·rank/org pinned under the topbar while the sheet scrolls), then grouped rows: **Overview** (Frontier Elo, rank, benchmarks tested, categories covered, benchmark wins over the contested set, Elo-per-$ value), **Specifications** (org/family/release+age/status/succeeds/weights/license+class/arch/params/context/modalities/languages/API), **Price & deployment** (in/out/blended-3:1 $/Mtok, VRAM Q4+FP16, smallest fitting GPU, ~tok/s on a 4090, quants, real outbound links), a **capabilities matrix** (6 caps as ✓/✕ in slot colors), then **benchmarks grouped by category** — each group header carries the models' `categoryIdx` + inline bars, each bench row shows the score, the gap to the compared-set leader (`−Δ`), a provenance flag for non-independent sources, and a right-aligned **field-best column** (the tracked record over all 584 models, so compared numbers read against the frontier, not in a vacuum). Best value per row highlights in the winner's slot color (tied extrema share it; <2 covered = no highlight). A coverage filter (`All tested / Shared / One-sided`) restricts the bench rows — "shared" answers the head-to-head question, "one-sided" exposes asymmetric coverage honestly (D20 spirit). **4th slot (D, `--slotd` amber)** joins A/B/C with positional URL params (`?m=a,b,,d` keeps interior empties); a slug already in another slot is excluded from each picker (self-compare was never the intent). Right rail: **Head-to-head verdict card** (rating leader + gap, per-category lead tally, shared-bench record incl. ties, cheapest by blended price, newest, runs-locally list, best-value pick) answering "who wins" at a glance; the D24 adaptive radar + legend kept verbatim; a **Field-context scatter** plots the whole priced field with the compared models labeled (click adds to the first open slot, else opens the model page). Copy-link button shares the exact `?m=` state. Layout `xl:grid-cols-[1fr_360px]`, page `max-w-[1400px]`. |
...
battles      = per benchmark, every pair of models that BOTH hold a headline score
               ...
               exact tie = half-win each; each battle weighted 1/sqrt(n_c) where n_c =
               benchmarks of that battle's CATEGORY the pair shares (D26 — a shared
               domain contributes sqrt(n_c) total match weight, so evidence still
               accumulates but no domain dominates a pairing linearly);
               aggregated into canonical PairRecords
...
- **Scatter** (quality vs price, viewBox 720×320): `x = 46 + (log10(p) − log10(0.06)) / (log10(200) − log10(0.06)) × (712 − 46)`; y-axis is **data-driven** — `fitYWindow` pads the plotted Frontier Elo range and lays ~6 round interior ticks, unclamped (the rating has no fixed domain; D21); x ticks `{$0.1,$1,$10,$100}`. A **Pareto frontier** guide line (`paretoFrontier`, D27) overlays the plotted set: the non-dominated upper-left envelope (cheaper-and-higher-Elo dominates), returned in ascending-price order and drawn as an `--acc` polyline beneath the dots — recomputed from the visible points, so a legend camp filter re-fits it like the y-window.
