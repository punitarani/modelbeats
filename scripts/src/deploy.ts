...
/**
 * Credential-gated deploy (plan commit 33). Runs from a human's shell for the first
 * production deploy, then from GitHub Actions CD on every merge to main. See docs/DEPLOY.md.
 *
 *   bun run deploy:production   migrate remote D1 → build → deploy Worker → publish data
 *
 * Production-only by design (one D1 + one KV + one Worker); local miniflare covers pre-prod.
 */
...
This deploy is credential-gated on purpose. To ship production:
  1. Create the resources once:   wrangler d1 create rankedmodel
                                  wrangler kv namespace create CATALOG
  2. Paste the returned ids into apps/web/wrangler.jsonc under env.production
     (replace the REPLACE_AT_DEPLOY_production_* placeholders).
