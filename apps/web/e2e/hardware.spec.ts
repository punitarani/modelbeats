...
test.describe('hardware explorer', () => {
  test('M3 Max budget grades llama-3.3-70b comfortable and kimi-k2 wont-run', async ({ page }) => {
    ...
    await expect(llama.getByTestId('verdict-fits-comfortably')).toBeVisible() // 42×1.08=45.4 ≤ 76.8
    const kimi = page.getByTestId('fit-row').filter({ hasText: 'Kimi K2' }).first()
    await expect(kimi.getByTestId('verdict-wont-run')).toBeVisible() // 600×1.08=648 GB
  })

  test('manual 12 GB budget: gpt-oss-20b (Medium) is offload-partial (ratio 1.17)', async ({
    page,
  }) => {
    await gotoHydrated(page, '/hardware?gpu=manual&vram=12')
    const row = page.getByTestId('fit-row').filter({ hasText: 'gpt-oss-20b (Medium)' })
    ...
    await expect(
      page.getByTestId('fit-row').filter({ hasText: 'gpt-oss-20b (Medium)' }),
    ).toHaveCount(0)
  })
  ...
  test('rtx4090 shows measured tok/s and links into the explorer facet', async ({ page }) => {
    ...
    const row = page.getByTestId('fit-row').filter({ hasText: 'gpt-oss-20b (Medium)' })
    await expect(row).toContainText('~225')
  ...
  test('inverse mode grades one model across every profile', async ({ page }) => {
    await gotoHydrated(page, '/hardware?mode=model&model=gpt-oss-20b-medium')
    await expect(
