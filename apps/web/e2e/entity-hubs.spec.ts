...
test.describe('organization + family hubs', () => {
  ...
  test('/families/claude-4 shows progression and succession deltas', async ({ page }) => {
    ...
    await expect(page.getByTestId('family-sparkline').locator('circle')).toHaveCount(5)
  ...
  test('model detail links through to its family and back to the org', async ({ page }) => {
